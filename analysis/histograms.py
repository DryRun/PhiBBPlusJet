...
import sys
import ROOT
from DAZSLE.DAZSLECommon.analyzer.analysis_base import AnalysisBase
from DAZSLE.DAZSLECommon.analyzer.event_selector import *
...
class Histograms(AnalysisBase):
	...
	def initialize_weight_tools(self):
		# Pileup weight stuff
		f_pu = TFile.Open("$CMSSW_BASE/src/DAZSLE/DAZSLECommon/data/puWeights_All.root", "read")
		...
		if self._jet_type == "AK8":
			f_trig = ROOT.TFile.Open("$CMSSW_BASE/src/DAZSLE/DAZSLECommon/data/RUNTriggerEfficiencies_AK8_SingleMuon_Run2016_V2p1_v03.root", "read")
			...
			f_trig = ROOT.TFile.Open("$CMSSW_BASE/src/DAZSLE/DAZSLECommon/data/RUNTriggerEfficiencies_CA15_SingleMuon_Run2016_V2p4_v08.root", "read")
		...
		f_mutrig_GH = ROOT.TFile.Open("$CMSSW_BASE/src/DAZSLE/DAZSLECommon/data/EfficienciesAndSF_Period4.root", "read")
		...
		f_mutrig_BCDEF = ROOT.TFile.Open("$CMSSW_BASE/src/DAZSLE/DAZSLECommon/data/EfficienciesAndSF_RunBtoF.root", "read")
		...
		f_muid_GH = ROOT.TFile.Open("$CMSSW_BASE/src/DAZSLE/DAZSLECommon/data/EfficienciesAndSF_GH.root", "read")
		...
		f_muid_BCDEF = ROOT.TFile.Open("$CMSSW_BASE/src/DAZSLE/DAZSLECommon/data/EfficienciesAndSF_BCDEF.root", "read")
		...
		f_muiso_GH = ROOT.TFile.Open("$CMSSW_BASE/src/DAZSLE/DAZSLECommon/data/EfficienciesAndSF_ISO_GH.root", "read")
		...
		f_muiso_BCDEF = ROOT.TFile.Open("$CMSSW_BASE/src/DAZSLE/DAZSLECommon/data/EfficienciesAndSF_ISO_BCDEF.root", "read")
		...
		for selection in self._selections:
			...
			for box in self._boxes:
				...
				if self._do_ps_weights:
					for ipsweight in xrange(0, 20):
						self._selection_histograms[selection].AddTH2F("{}_pt_vs_msd_psweight{}".format(box, ipsweight), "; {} m_{{SD}}^{{PUPPI}} (GeV); {} p_{{T}} (GeV)".format(self._jet_type, self._jet_type), "m_{SD}^{PUPPI} [GeV]", 80, 40, 600, "p_{T} [GeV]", 24, 0., 1200.)
						self._selection_histograms[selection].AddTH1F("{}_n2_psweight{}".format(box, ipsweight), "n2", "N_{2}^", 160, -1.0, 1.0)
			...
			for selection in self._selections:
				...
				if self._event_selectors[selection].event_pass():
					for box in event_boxes:
						...
						if self._do_ps_weights:
							for ipsweight in xrange(0, 20):
								self._selection_histograms[selection].GetTH2F("{}_pt_vs_msd_psweight{}".format(box, ipsweight)).Fill(fatjet_msd, fatjet_pt, event_weight * self._data.psWeights[ipsweight])
								self._selection_histograms[selection].GetTH1F("{}_n2_psweight{}".format(box, ipsweight)).Fill(fatjet_n2, event_weight * self._data.psWeights[ipsweight])
								self._selection_histograms[selection].GetTH1F("{}_n2ddt_psweight{}".format(box, ipsweight)).Fill(fatjet_n2ddt, event_weight * self._data.psWeights[ipsweight])
...
if __name__ == "__main__":
	...
	parser.add_argument('--year', type=int, default=2016, help="AK8 or CA15")
