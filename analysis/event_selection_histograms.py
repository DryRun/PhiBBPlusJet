...
class EventSelectionHistograms(AnalysisBase):
	...
	def start(self):
		...
		if self._do_optimization:
			for tau21_ddt_cut in [0.4, 0.5, 0.55, 0.6, 0.7]:
		...
		for entry in xrange(first_event, limit_nevents):
			...
			for selection in self._selections:
				...
				if self._event_selectors[selection].event_pass():
					...
					self._selection_histograms[selection].GetTH2D("met_msd_pass").Fill(fatjet_msd, self._data.pfmet, event_weight)
					self._selection_histograms[selection].GetTH2D("met_msd_fail").Fill(fatjet_msd, self._data.pfmet, event_weight)
