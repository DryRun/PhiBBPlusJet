...
class HbbHistograms(AnalysisBase):
	def __init__(self, sample_name, tree_name="otree", jet_type="AK8", jet_ordering="pt"):
		super(HbbHistograms, self).__init__(tree_name=tree_name)
	...
	def set_data_source(self, data_source):
		print "[HbbHistograms::set_data_source] INFO : Setting data source to " + data_source
		if not data_source in ["data", "simulation"]:
			print "[HbbHistograms] ERROR : Data source must be data or simulation."
			sys.exit(1)
	...
	def add_file(self, filename):
		super(HbbHistograms, self).add_file(filename)
		f = ROOT.TFile.Open(filename, "READ")
		if f.Get("NEvents").Integral() == 0:
			print "[HbbHistograms::add_file] ERROR : NEvents.Integral() == 0 for file " + filename
			sys.exit(1)
	...
	def run(self, max_nevents=-1, first_event=0):
		...
		print "[HbbHistograms::run] INFO : Running loop over tree from event {} to {}".format(first_event, limit_nevents - 1)
...
if __name__ == "__main__":
	...
	for sample in samples:
		...
		limit_histogrammer = HbbHistograms(sample, tree_name=tree_name, jet_type=args.jet_type)
