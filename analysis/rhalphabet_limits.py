...
def CreateCombineProject(region_names, pass_histograms, fail_histograms, background_names=[], signal_name="", data_name="data_obs", jet_type="AK8", datacard_directory="/tmp/"):
	...
	for region_name in region_names:
		...
		for background_name in background_names:
			if background_name == "qcd":
				...
				y_value = (1. * pt_categories[ptcat][0] + 2. * pt_categories[ptcat][1]) / 3.
				region_containers[region_name].add_rhbackground("qcd", y_value, n_x=1, n_y=1, x_range=[-7., 0.], y_range=[400., 1000.])
			else:
		...
		region_containers[region_name].add_data("data_obs", pass_histograms[region_name][data_name], fail_histograms[region_name][data_name])
		# def add_signal(self, signal_name, pass_hist, fail_hist, normalization_var=None):
		region_containers[region_name].add_signal(signal, pass_histograms[region_name][signal_name], fail_histograms[region_name][signal_name])
	...
	for dbtag_wp in dbtag_wps:
		for n2wp_dbpass in n2ddt_wps:
			for n2wp_dbfail in n2ddt_wps:
				...
				optimization_wps[opt_wp_string] = (dbtag_wp, n2wp_dbpass, n2wp_dbfail)
	# Also non-b-tagged
	for n2wp in n2ddt_wps:
		opt_wp_string = "dbtagnone_n2wp{}".format(n2wp)
		optimization_wps[opt_wp_string] = (0.7, n2wp, n2wp)
			...
			for signal in signals:
				for opt_wp_name, opt_wp_parameters in optimization_wps.iteritems():
					...
					for box in ["passn2_passdbtag", "passn2_faildbtag", "failn2_passdbtag", "failn2_faildbtag"]:
						...
						# Slice 2D histograms
						for process in backgrounds + [signal]:
							h2d_slices[box][process] = SliceSignalHistogram(h2d[process], pt_categories=default_pt_categories)

						# Data histogram = pseudodata from background
						for process in backgrounds:
							if not "data_obs" in h2d:
								h2d[box]["data_obs"] = h2d[process].Clone()
								h2d[box]["data_obs"].SetName("data_obs_{}_pt_vs_msd".format(box))
							else:
								h2d[box]["data_obs"].Add(h2d[process])
						h2d_slices[box]["data_obs"] = SliceSignalHistogram(h2d["data_obs"], pt_categories=default_pt_categories)
					# End loop over boxes
					...
					for btag_cat in ["passdbtag", "faildbtag"]:
						...
						for box in [pass_box, fail_box]:
							for xbin in xrange(1, h2d_slices["data_obs"][ptcat].GetNbinsX()+1):
								bin_mass = h2d_slices[box]["data_obs"][ptcat].GetXaxis().GetBinCenter(xbin)
								...
								if bin_rho < rho_range[0] or bin_rho > rho_range[1]:
									for process in backgrounds + ["data_obs", signal]:
										h2d_slices[box][process][ptcat].SetBinContent(xbin, 0.)
										h2d_slices[box][process][ptcat].SetBinError(xbin, 0.)
						# End loop over N2 pass/fail

						# If the fail bin 0 entries, set pass=0.
						if h2d_slices[fail_box]["data_obs"][ptcat].GetBinContent(xbin) == 0:
							h2d_slices[pass_box]["data_obs"][ptcat].SetBinContent(xbin, 0)
							h2d_slices[pass_box]["data_obs"][ptcat].SetBinError(xbin, 0)
					...
					CreateCombineProject(region_names, pass_histograms, fail_histograms, jet_type=jet_type, datacard_directory=datacard_directory)
